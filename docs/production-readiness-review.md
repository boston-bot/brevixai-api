...
| --- | --- | --- | --- |
| `brevixai-api` | `dev` | Dirty | Existing Rex chat / agent runner changes are present. |
...
| Auth, tenancy, and authorization | Not started | Verify cross-company isolation, admin scope, route guards, and internal service auth. |
...
| PRR-009 | Medium | API CORS | Open | API CORS config always includes localhost origins, including production, and does not fail closed when `APP_FRONTEND_URL`/`CORS_ALLOWED_ORIGINS` are missing. | `brevixai-api/config/cors.php`. | Add production CORS validation and document exact AWS frontend origin. |
